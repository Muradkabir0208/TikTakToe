...
Button::Button(std::string text, sf::Texture texture, sf::Vector2f position,
...
void Button::setFont(const sf::Font &font) { text.setFont(font); }

void Button::setText(const std::string &text) { this->text.setString(text); }
...
{
    auto ir = sprite.getTextureRect();
    return {sprite.getPosition().x - sprite.getOrigin().x,
            sprite.getPosition().y - sprite.getOrigin().y, (float)ir.width,
            (float)ir.height};
}
