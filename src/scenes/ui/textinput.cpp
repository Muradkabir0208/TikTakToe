...
void TextInput::draw(sf::RenderWindow &window) {
  ...
  if (showUnderscore && canCharFit() && !limitReached)
    text.setString(raw + "_");
  else
    text.setString(raw);
...
std::string TextInput::getInput() { return raw; }
...
void TextInput::setSize(sf::FloatRect floatRect) {
  ...
  text.setPosition(
      std::lroundf(size.left + PADDING),
      std::lroundf(size.top + size.height / 2 - text.getCharacterSize() / 2));
}

bool TextInput::isEntered() const { return entered; }
