...
NetworkScene::NetworkScene(GameInfo *gameInfo) : gameInfo(gameInfo)
{
    ...
    ipText.setPosition(std::lroundf(Window::VWIDTH / 2),
                       std::lroundf(Window::VHEIGHT / 2 - 50));
    ...
    loadText.setPosition(std::lroundf(Window::VWIDTH / 2),
                         std::lroundf(Window::VHEIGHT / 2 + 100));
...
void NetworkScene::update(float delta)
{
    if (threadInfo.state == ThreadInfo::IDLE)
    {
        ...
        if (gameInfo->networkType == CLIENT)
            threadInfo.state = ThreadInfo::GATHERING_INFO;
    }
    ...
    if (threadInfo.state == ThreadInfo::SETUP)
    {
        ...
        while (ti->state != ThreadInfo::CONNECTED) {
          if (ti->socket.connect(ti->ip, ThreadInfo::PORT) ==
              sf::Socket::Done) {
            ti->state = ThreadInfo::CONNECTED;
            ti->socket.setBlocking(false);
