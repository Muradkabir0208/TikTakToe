  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      ...
      mapButtons[i][j].setPosition(
          {(float)i * (layoutTex.getSize().x / 3.0f + 3.0f) +
               layout.getPosition().x - layout.getOrigin().x + sr.x / 2,
           (float)j * (layoutTex.getSize().y / 3.0f + 3.0f) +
               layout.getPosition().y - layout.getOrigin().y + sr.y / 2});
...
void GameScene::update(float delta) {
  if (moveMade) {
    ...
    if (player == Map::C_CROSS)
      player = Map::C_CIRCLE;
    else if (player == Map::C_CIRCLE)
      player = Map::C_CROSS;
  ...
  // TODO: implement a better way of reseting a game, eg. send a reset request
  // and check to server and client, or a reset button
  if (gameOver) {
    time += delta;
...
void GameScene::draw(sf::RenderWindow &window) {
  ...
  if (winner == 0 && !map.isFull()) {
    ...
    st << "  -  Reset in " << std::fixed << std::setprecision(1)
       << (RESET_TIME - time) << "s";
    gameOver = true;
  }
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      ...
      if (c == Map::C_CROSS)
        s.setTexture(
            crossTex,
            true); // very inefficient, since we are binding them so often
      else if (c == Map::C_CIRCLE)
        s.setTexture(circleTex, true);
      else
        continue;
